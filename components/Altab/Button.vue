<template>
  <NuxtLink
    v-if="Type == 'link'"
    :to="Route"
    :class="`self-center transition-colors duration-300 ease-in transform rounded-xl hover:cursor-pointer px-2 h-12 text-center flex flex-col justify-center ${add_class}`"
  >
    {{ Title }}
  </NuxtLink>
  <div v-else-if="Type=='bilink'" class="self-center flex flex-row">
    <NuxtLink
      :to="Route"
      :class="`self-center transition-colors duration-300 ease-in transform rounded-l-xl hover:cursor-pointer px-2 h-12 text-center flex flex-col justify-center ${add_class}`"
    >
      {{ Title }}
    </NuxtLink>
    <NuxtLink
      :to="biRoute"
      :class="`self-center transition-colors duration-300 ease-in transform rounded-r-xl hover:cursor-pointer px-2 h-12 text-center flex flex-col justify-center ${add_class}`"
    >
      {{ biTitle }}
    </NuxtLink>
  </div>
